...
class YAMLConstructor:
    ...
    def fix_signature(cls, signature: BoundArguments) -> BoundArguments:
        for name, value in signature.arguments.items():
            try:
                ...
                hint_is_builtin = any(
                    hint.__module__.startswith(mod)
                    for mod in ("builtins", "typing", "collections")
                )
                hint_is_str = issubclass(hint, str)
                hint_is_callable = issubclass(hint, (type, tp.Callable))
                target_nodetype = (
                    (hint in (tp.Type, tp.Callable) or hint_is_str)
                    and yaml.ScalarNode
                    or hint_is_builtin
                    and issubclass(hint, tp.Mapping)
                    and yaml.MappingNode
                    or hint_is_builtin
                    and issubclass(hint, tp.Iterable)
                    and yaml.SequenceNode
                    or yaml.Node
                )
                ...
                for node in valueiter(value):
                    ...
                    if (
                        ...
                        and (
                            not hint_is_builtin
                            or target_nodetype in (yaml.ScalarNode, yaml.Node)
                        )
                        ...
                        if hint_is_callable:
                            ...
                            node.tag = cls.type_to_tag.get(
                                hint, f"!py:{hint.__module__}.{hint.__name__}"
                            )
    ...
    @classmethod
    def get_args_kwargs(
        cls, loader: yaml.Loader, node: yaml.Node, signature: Signature = free_signature
    ) -> tp.Optional[BoundArguments]:
        if isinstance(node, yaml.ScalarNode):
            # This sometimes fails because of ruamel/yaml/resolver.py line 370
            node.tag = loader.resolver.resolve(
            ...
            # Construct the keys
            kwargs = {
                loader.construct_object(key, deep=True): val for key, val in node.value
            }

            args = kwargs.setdefault(
                "__args", yaml.SequenceNode("tag:yaml.org,2002:seq", [])
            )
            args_is_seq = (
                isinstance(args, yaml.SequenceNode)
                and args.tag == "tag:yaml.org,2002:seq"
            )
            if args_is_seq:
        ...
        subnode_values = {n: loader.construct_object(n, deep=True) for n in subnodes}
...
class PrefixedTensorYAMLConstructor(YAMLConstructor):
    ...
    def construct(cls, loader: yaml.Loader, suffix: str, node: yaml.Node, **kwargs):
        ...
        if not isinstance(getattr(torch, suffix), torch.dtype):
            raise ValueError(
                f'In tag "!torch:{suffix}", "{suffix}" is not a valid torch.dtype.'
            )
    ...
    for spec in specs:
        if isinstance(spec, str):
            ...
            vars(builtins).update(
                {oname: getattr(module, iname) for iname, oname in fromlist}
            )
...
class MyYAML(yaml.YAML):
    ...
    def __init__(self, base_dir: os.PathLike = None):
        ...
        c.add_multi_constructor(
            "!Stochastic:", PrefixedStochasticYAMLConstructor.construct
        )
        ...
        c.add_constructor(
            "!SemiInfiniteSampler", YAMLConstructor.apply(SemiInfiniteSampler)
        )
